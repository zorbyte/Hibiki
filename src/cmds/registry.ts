...
export function find(nameOrAlias: string) {
    let foundCmd = commands.get(nameOrAlias);
...
export function runner(...runners: Runner[]) {
    async function compiledRunner(ctx: Context) {
        return await Promise.all(runners.map(async rn => {
            return await rn(ctx)
        }));
    }
...
/** Registers a command */
export function register(callKeys: [name: string, ...aliases: string[]], runner: Runner, opts: RegisterOpts = {}) {
